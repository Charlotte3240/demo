...
const DelayMethod = (time : number) :MethodDecorator =>(...args :any[])=>{
...
// 装饰器工厂
const ErrorDecorator = (title : string, message : string):MethodDecorator => (
    target, propertyKey, descriptor:PropertyDescriptor)=>{
    const method = descriptor.value
    descriptor.value = ()=>{
        try{
...
const  HCParamDecorator : ParameterDecorator = (target, propertyKey, parameterIndex) => {
