...
# 查询商品超过两个的品牌 对分组结果进行筛选用 having
select COUNT(id) as c,  brandName  from goods g
group by brandName 
HAVING c > 2

# 查询本周一迟到超过两次的同学
# 先查询迟到的数据，再对person_id 进行分组，再对分组的数据中的次数进行筛选
SELECT person_id , COUNT(id) as c FROM attendance a 
...
# 查询本周准时次数最多的同学
SELECT COUNT(id) as c , person_id from attendance a 
WHERE  time(create_at) <= '08:30:00'
AND date(create_at) >= date(DATE_ADD(NOW(),INTERVAL 0-WEEKDAY(NOW()) DAY))
GROUP  BY  person_id 
ORDER BY c DESC 
...
# 查询本周哪天迟到的人最多
select  COUNT(id) as t, WEEKDAY(create_at) as d from attendance a 
WHERE  time(create_at) > '08:30:00'
AND date(create_at) >= date(DATE_ADD(NOW(),INTERVAL 0-WEEKDAY(NOW()) DAY))
group by d 
ORDER by t DESC 
