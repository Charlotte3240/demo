...
- (void)progressChangedWithWKWebViewProgress:(double)progress{
    self.strokeEnd = progress;
    if (progress == 1){
        __weak DKProgressLayer *weakSelf = self;
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            weakSelf.hidden = YES;
            weakSelf.stepWidth = 0.001;
            weakSelf.strokeEnd = 0;
            weakSelf.gradientLayer.locations = @[@(self.strokeEnd/2), @(self.strokeEnd)];
            weakSelf.gradientLayer.frame = CGRectMake(0, 0, 0, weakSelf.lineHeight);
        });
